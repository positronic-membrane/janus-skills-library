"""Staging test for Scan Workspace skill."""
...
def test_skill_entry_point_defined():
    ...
    spec = importlib.util.spec_from_file_location(
        "skill", Path(__file__).parent / "scan_workspace.py"
    )
    mod = importlib.util.module_from_spec(spec)
    mod.sdk = _make_sdk()
    spec.loader.exec_module(mod)
    assert hasattr(mod, "scan_workspace")
    assert callable(getattr(mod, "scan_workspace"))
