"""Staging test for Decay Self-Model skill."""
...
def test_skill_entry_point_defined():
    ...
    spec = importlib.util.spec_from_file_location(
        "skill", Path(__file__).parent / "decay_self_model.py"
    )
    mod = importlib.util.module_from_spec(spec)
    mod.sdk = _make_sdk()
    spec.loader.exec_module(mod)
    assert hasattr(mod, "decay_self_model")
    assert callable(getattr(mod, "decay_self_model"))
