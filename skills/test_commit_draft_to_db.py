"""Staging test for Commit Draft to DB skill."""
...
def test_skill_entry_point_defined():
    ...
    spec = importlib.util.spec_from_file_location(
        "skill", Path(__file__).parent / "commit_draft_to_db.py"
    )
    mod = importlib.util.module_from_spec(spec)
    mod.sdk = _make_sdk()
    spec.loader.exec_module(mod)
    assert hasattr(mod, "commit_draft_to_db")
    assert callable(getattr(mod, "commit_draft_to_db"))
