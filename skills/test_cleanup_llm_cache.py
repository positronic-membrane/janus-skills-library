"""Staging test for Cleanup LLM Cache skill."""
...
def test_skill_entry_point_defined():
    ...
    spec = importlib.util.spec_from_file_location(
        "skill", Path(__file__).parent / "cleanup_llm_cache.py"
    )
    mod = importlib.util.module_from_spec(spec)
    mod.sdk = _make_sdk()
    spec.loader.exec_module(mod)
    assert hasattr(mod, "cleanup_llm_cache")
    assert callable(getattr(mod, "cleanup_llm_cache"))
