"""Staging test for Checkout DB to Draft skill."""
...
def test_skill_entry_point_defined():
    ...
    spec = importlib.util.spec_from_file_location(
        "skill", Path(__file__).parent / "checkout_db_to_draft.py"
    )
    mod = importlib.util.module_from_spec(spec)
    mod.sdk = _make_sdk()
    spec.loader.exec_module(mod)
    assert hasattr(mod, "checkout_db_to_draft")
    assert callable(getattr(mod, "checkout_db_to_draft"))
