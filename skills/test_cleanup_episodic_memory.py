"""Staging test for Cleanup Episodic Memory skill."""
...
def test_skill_entry_point_defined():
    ...
    spec = importlib.util.spec_from_file_location(
        "skill", Path(__file__).parent / "cleanup_episodic_memory.py"
    )
    mod = importlib.util.module_from_spec(spec)
    mod.sdk = _make_sdk()
    spec.loader.exec_module(mod)
    assert hasattr(mod, "cleanup_episodic_memory")
    assert callable(getattr(mod, "cleanup_episodic_memory"))
